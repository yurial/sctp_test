#ifndef EXT_ESCAPE_HPP
#define EXT_ESCAPE_HPP
...
escape_iterator<T>::escape_iterator(container_type& container, const escape_mask_type& mask):
    m_container( container ), m_escape_mask( mask )
    {
    }

template <class T>
escape_iterator<T>::escape_iterator(container_type& container, const std::string& chars):
    m_container( container ), m_escape_mask()
    {
    std::for_each( chars.begin(), chars.end(), [&](char chr)
        {
        m_escape_mask.set( static_cast<unsigned char>( chr ) );
        } );
    }
...
T escape(const T& str, const T& chars)
    ...
    std::copy( str.begin(), str.end(), ext::escape_iterator<T>( result, chars ) );
    return result;
    }
